...
return ( as.vector(-2*X_i*(y_i-sum(X_i*beta))) )
}
...
while (!converged && iter < maxiter) {
    ...
    for(i in 1:dim(X)[1]){
        beta <- beta - alpha*gradient(beta,y_shuf[i],as.numeric(X_shuf[i,]))
    }
    # Check convergence: relative change in full objective
    obj_curr <- objfun(beta,y,X)
    if (abs(obj_curr-obj_prev)/(1+abs(obj_prev)) < 1e-10) converged <- TRUE
    obj_prev <- obj_curr
    if (iter%%1000==0) {
        print(beta)
    }
...
- convergence can be finnicky due to noise in gradients

---
# Further SGD considerations

- must slowly decrease $\alpha$ (the learning rate) to achieve convergence: the schedule must satisfy the .hi[Robbins-Monro conditions] $\sum \alpha_t = \infty$ (enough total progress to reach the minimum) and $\sum \alpha_t^2 < \infty$ (noise eventually shrinks). 
    - Geometric decay like $\alpha / 1.0005^t$ fails the first condition
    - inverse-time decay $\alpha / (1 + ct)$ satisfies both

- unlike batch GD (where the full gradient is exactly zero at the minimum), per-observation gradients are noisy even at the optimum, so checking parameter change can fire prematurely
    - checking the relative change in the full objective function is more reliable
